...
async function getTarefa(_req, res) {
    const data = await (0, knex_1.db)('tarefa').select('*').orderBy('id', 'desc');
...
async function createTarefa(req, res) {
    ...
    await (0, knex_1.db)('tarefa').insert(body);
    return res.status(201).json({ message: 'Trabalho criado com sucesso.' });
}
async function updateTarefa(req, res) {
    const id = Number(req.params.id);
    const body = req.body;
    const tarefa = await (0, knex_1.db)('tarefa').where({ id }).first();
    if (!tarefa) {
        return res.status(404).json({ error: 'Tarefa não encontrada.' });
    }
    await (0, knex_1.db)('tarefa').where({ id }).update(body);
...
async function deleteTarefa(req, res) {
    ...
    await (0, knex_1.db)('tarefa').where({ id }).del();
